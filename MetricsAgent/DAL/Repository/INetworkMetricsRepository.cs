    ...
    public class NetworkMetricsRepository : INetworkMetricsRepository
    {
        ...
        public IList<NetworkMetrics> GetAll()
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                ...
                return connection.Query<NetworkMetrics>("SELECT Id, Time, Value FROM cpumetrics").ToList();
            }
        }
        ...
        public NetworkMetrics GetById(int id)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.QuerySingle<NetworkMetrics>("SELECT Id, Time, Value FROM cpumetrics WHERE id=@id",
                    new { id = id });
            }
        }
    }
