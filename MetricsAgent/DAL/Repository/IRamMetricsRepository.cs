    ...
    public class RamMetricsRepository : IRamMetricsRepository
    {
        ...
        public IList<RamMetrics> GetAll()
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                ...
                return connection.Query<RamMetrics>("SELECT Id, Time, Value FROM cpumetrics").ToList();
            }
        }
        ...
        public RamMetrics GetById(int id)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.QuerySingle<RamMetrics>("SELECT Id, Time, Value FROM cpumetrics WHERE id=@id",
                    new { id = id });
            }
        }
    }
